...
- `firecrawl`, `exa`, `alphavantage`, `polymarket`, `hyperliquid`, and `alpaca` are configured but disabled until credentials are filled in `{{HERMES_FINANCE_HOME}}/.env` and enabled explicitly.
...
- Vault operations: filesystem vault access by default; if the operator already has Nexus MCP in their environment, prefer Nexus for broader Obsidian workflows instead of a separate Obsidian REST MCP.
