...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Solthodox/hermes-finance",
    ...
    lines.append(f"# Hermes Finance v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Hermes Finance. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Hermes Finance Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Hermes Finance Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Hermes Finance v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Hermes Finance v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Hermes Finance v{new_version} ({calver_date})' "
