...
# Configuration
REPO_URL_SSH="git@github.com:Solthodox/hermes-finance.git"
REPO_URL_HTTPS="https://github.com/Solthodox/hermes-finance.git"
HERMES_HOME="${HERMES_HOME:-$HOME/.hermes-finance}"
...
#   code at /usr/local/lib/hermes-finance, command at /usr/local/bin/hermes-finance,
#   data still at /root/.hermes-finance (HERMES_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.hermes-finance/hermes-finance"
            echo "                   default (root, Linux): /usr/local/lib/hermes-finance"
            echo "  --hermes-home PATH  Data directory (default: ~/.hermes-finance, or \$HERMES_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hermes installs the code under"
            echo "  /usr/local/lib/hermes-finance and links the command into"
            echo "  /usr/local/bin/hermes-finance (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HERMES_HOME"
            echo "  (default /root/.hermes-finance).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HERMES_HOME/hermes-finance are preserved in-place."
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ Hermes Finance Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  A pragmatic, analytic finance AI agent fork by Solthodox.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# Decide where the repo checkout + venv live, and where # the `hermes-finance` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HERMES_HOME/hermes-finance
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HERMES_HOME/hermes-finance
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/hermes-finance
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HERMES_HOME/hermes-finance — then preserve it)
#
...
resolve_install_layout() {
    ...
    if is_termux; then
        INSTALL_DIR="$HERMES_HOME/hermes-finance"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HERMES_HOME/hermes-finance/.git" ]; then
            INSTALL_DIR="$HERMES_HOME/hermes-finance"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/hermes-finance)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/hermes-finance"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/hermes-finance"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/Solthodox/hermes-finance/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_node() {
    ...
    log_info "Extracting to ~/.hermes-finance/node/..."
    ...
    # Place into ~/.hermes-finance/node/ and symlink binaries to ~/.local/bin/
    ...
    log_success "Node.js $installed_ver installed to ~/.hermes-finance/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Hermes Finance itself does not require or retain root access."
...
EOF
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        log_info "hermes-finance not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Hermes Finance — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Hermes Finance — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Hermes Finance — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so hermes-finance works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create .env at ~/.hermes-finance/.env (top level, easy to find)
    if [ ! -f "$HERMES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HERMES_HOME/.env"
            log_success "Created ~/.hermes-finance/.env from template"
        else
            touch "$HERMES_HOME/.env"
            log_success "Created ~/.hermes-finance/.env"
        fi
    else
        log_info "~/.hermes-finance/.env already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.hermes-finance/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.hermes-finance/"

    # Seed bundled skills into ~/.hermes-finance/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.hermes-finance/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.hermes-finance/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HERMES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$HERMES_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.hermes-finance/skills/"
        fi
    fi
}
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'hermes-finance setup' after install."
        return 0
    fi
    ...
    # Run hermes-finance setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'hermes-finance gateway install' later."
        return 0
    fi
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: hermes-finance gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: hermes-finance gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.hermes-finance/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: hermes-finance gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: hermes-finance gateway"
    fi
}
