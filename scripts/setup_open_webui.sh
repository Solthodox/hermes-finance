...
# Bootstrap Open WebUI against Hermes Finance's OpenAI-compatible API server.
#
...
#   HERMES_API_MODEL_NAME='Hermes Finance'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Hermes Finance WebUI}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Hermes Finance}"
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Hermes Finance
...
main() {
  ...
  hermes-finance gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${HERMES_API_CONNECT_HOST}:${HERMES_API_PORT}/health" >/dev/null; then
    ...
    nohup hermes-finance gateway run >/dev/null 2>&1 &
    sleep 6
  fi
