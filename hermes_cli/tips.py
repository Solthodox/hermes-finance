...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hermes-finance -c.",
    ...
    # --- CLI Flags ---
    "hermes-finance -c resumes your most recent CLI session. hermes-finance -c \"project name\" resumes by title.",
    "hermes-finance -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hermes-finance -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hermes-finance chat -t web,terminal enables only specific toolsets for a focused session.",
    "hermes-finance chat -s github-pr-workflow preloads a skill at launch.",
    "hermes-finance chat -q \"query\" runs a single non-interactive query and exits.",
    "hermes-finance chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "hermes-finance chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hermes-finance --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hermes-finance chat --source telegram tags the session for filtering in hermes-finance sessions list.",
    "hermes-finance -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "hermes-finance doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hermes-finance dump outputs a compact setup summary — great for bug reports.",
    "hermes-finance config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hermes-finance config edit opens config.yaml in your default editor.",
    "hermes-finance config check scans for missing or stale configuration options.",
    "hermes-finance sessions browse opens an interactive session picker with search.",
    "hermes-finance sessions stats shows session counts by platform and database size.",
    "hermes-finance sessions prune --older-than 30 cleans up old sessions.",
    "hermes-finance skills search react --source skills-sh searches the skills.sh public directory.",
    "hermes-finance skills check scans installed hub skills for upstream updates.",
    "hermes-finance skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hermes-finance skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hermes-finance mcp add github --command npx adds MCP servers from the command line.",
    "hermes-finance mcp serve runs Hermes itself as an MCP server for other agents.",
    "hermes-finance auth add lets you add multiple API keys for credential pool rotation.",
    "hermes-finance completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hermes-finance logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hermes-finance backup creates a zip backup of your entire Hermes home directory.",
    "hermes-finance profile create coder creates an isolated profile that becomes its own command.",
    "hermes-finance profile create work --clone copies your current config and keys to a new profile.",
    "hermes-finance update syncs new bundled skills to ALL profiles automatically.",
    "hermes-finance gateway install sets up Hermes as a system service (systemd/launchd).",
    "hermes-finance memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hermes-finance webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hermes-finance tools disables unused tools, hermes-finance skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "hermes-finance models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hermes-finance -w).",
    ...
    "Profile names become shell commands — 'hermes-finance profile create coder' creates the 'coder' command.",
    "hermes-finance profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "hermes-finance sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hermes-finance -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "hermes-finance skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .hermes-finance/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hermes-finance cron add --skill blogwatcher \"Check for new posts\".",
    ...
    "hermes-finance gateway install sets it up as a system service that starts on boot.",
    ...
    "SOUL.md at ~/.hermes-finance/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    "MCP servers auto-generate toolsets at runtime — hermes-finance tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.hermes-finance/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hermes-finance chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hermes-finance plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hermes-finance acp runs Hermes as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "hermes-finance login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "hermes-finance status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.hermes-finance/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.hermes-finance/SOUL.md to customize.",
    ...
    "The skills audit log at ~/.hermes-finance/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.hermes-finance/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hermes-finance profile create ls' would be rejected.",
    "hermes-finance profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "hermes-finance claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hermes-finance tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.hermes-finance/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.hermes-finance/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.hermes-finance/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.hermes-finance/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.hermes-finance/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'hermes-finance curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hermes-finance curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hermes-finance curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'hermes-finance auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'hermes-finance dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hermes-finance dashboard --tui embeds the full Hermes TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.hermes-finance/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.hermes-finance/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.hermes-finance/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.hermes-finance/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hermes-finance -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hermes-finance chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hermes-finance chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hermes-finance chat --ignore-user-config skips ~/.hermes-finance/config.yaml — reproducible bug reports and CI runs.',
    "hermes-finance chat --source tool tags programmatic chats so they don't clutter hermes-finance sessions list.",
    'hermes-finance dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hermes-finance sessions rename <ID> "new title" renames any past session; hermes-finance sessions delete <ID> removes one.',
    'hermes-finance import restores a session export or profile archive produced by sessions export or profile export.',
    'hermes-finance fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hermes-finance pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hermes-finance setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hermes-finance status --deep runs the full health sweep across every component; plain hermes-finance status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hermes-finance/plugins/ — trust-gated by design.',
    ...
    '--source tool chats are excluded from hermes-finance sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.hermes-finance/dashboard-plugins/.',
]
